...
public class ModsTomlData implements Serializable<String> {
    ...
    private Properties loadGradleProperties() {
        ...
        if (propsFile != null && propsFile.exists()) {
            try (FileInputStream fis = new FileInputStream(propsFile)) {
                ...
                System.err.println("Ошибка при чтении gradle.properties: " + e.getMessage());
            }
        } else {
            System.out.println("Предупреждение: Файл gradle.properties не найден вверх по дереву каталогов. Используются дефолтные значения.");
        }
        return props;
    }
    ...
    @Override
    public Path getPath() {
        return Path.of("META-INF", "mods.toml");
    }
    ...
    public ModsTomlData dependency(String modId, boolean mandatory, String versionRange, String ordering, String side) {
        this.dependencies.add(new Dependency(modId, mandatory, versionRange, ordering, side));
        return this;
    }
    ...
    private record Dependency(String modId, boolean mandatory, String versionRange, String ordering, String side) {}
}