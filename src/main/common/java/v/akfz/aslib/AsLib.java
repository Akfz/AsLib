...
import v.akfz.aslib.network.registry.IPacketRegistrar;
import v.akfz.aslib.util.scheduler.SchedulerListener;
...
public final class AsLib {
    ...
    public static void init() {
        ...
        for (IPacketRegistrar registrar : ServiceLoader.load(IPacketRegistrar.class, cl)) {
            try {
                registrar.register();
            } catch (Throwable t) {
                System.err.println("[AsLib] Packet registrar failed: " + registrar.getClass().getName());
                t.printStackTrace();
            }
        }

        EVENT_BUS.register(new SchedulerListener());
