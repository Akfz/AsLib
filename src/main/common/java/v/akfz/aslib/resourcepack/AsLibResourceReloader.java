...
/**
 * Custom reload listener that updates file caches and triggers registered
 * callbacks when resources/data are reloaded.
 */
public final class AsLibResourceReloader implements PreparableReloadListener {
    ...
    public CompletableFuture<Void> reload(PreparationBarrier synchronizer, ResourceManager manager,
        ...
        return synchronizer.wait(null).thenRunAsync(() -> {
            ...
            try {
                for (ResourceReloadListener globalListener : ReloadHelper.getGlobalListeners()) {
                    try {
