...
public final class AddResourcePack {
	...
	/**
	 * Registers a {@link PackResources} as a repository pack.
	 * {@code packType} picks between client assets and server data.
	 */
	public static void add(PackRepository manager, PackResources pack, Component description,
		...
		registerInternal(manager, id, displayName, alwaysEnabled, pos, pinned, source,
				description, name -> pack, packType);
	}

	/**
	 * Registers a {@link FileResourcePack} as a repository pack. The pack id and
	 * display name come from the pack itself.
	 */
