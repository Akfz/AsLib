...
public abstract class AbstractToolTip implements Renderable {
    ...
    protected RenderPart renderer = new DefaultToolTipRenderer();
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        if (renderX + totalWidth > screenWidth - 4) renderX = mouseX - totalWidth - 4;
        if (renderX < 4) renderX = 4;

        if (renderY + totalHeight > screenHeight - 4) renderY = screenHeight - totalHeight - 4;
    ...
    protected void renderAt(GuiGraphics context, int x, int y, int w, int h, float delta) {
        ...
        renderer.render(context, 0, 0, delta, x, y, w, h, extras);
    }
    ...
    public void setRenderer(RenderPart renderer) {
